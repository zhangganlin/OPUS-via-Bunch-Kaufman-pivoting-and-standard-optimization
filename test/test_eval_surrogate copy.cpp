...
void test_eval1(){
    ...
    double* groundtruth = (double*)malloc(sizeof(double));
    for(int i = 0; i < warmup_iter; i++){
        evaluate_surrogate_gt( x, points,  lambda_c, 1, N, d, groundtruth);
    }
    gt_start = start_tsc();
    ...
    cur_start = start_tsc();
    for(int i = 0; i < repeat; i++){
        evaluate_surrogate_gt( x, points,  lambda_c, 1, N, d, result);
    }
    cur_time = stop_tsc(cur_start);
    cout << "current result: "<< result[0] << endl;
